...
#elif (RH_PLATFORM == RH_PLATFORM_ARDUINO_NANO_EVERY)
{
    // Use one of the ATmega4809’s timer modules (for example, TCB0)
    // The idea is to set up TCB0 in periodic interrupt mode such that we get 8
    // interrupts per bit period. This code is similar in spirit to the ATTiny Mega branch.
    uint32_t compare_val = F_CPU / _speed / 8 - 1;
    // If the compare value is too large to fit in 16 bits, apply a prescaler (for example, DIV64)
    if (compare_val > 0xFFFF)
    {
        compare_val = F_CPU / _speed / 8 / 64 - 1;
        TCB0.CTRLA = TCB_CLKSEL_CLKTCA_gc; // Use clock from TCA with prescaler 64
    }
    else
    {
        TCB0.CTRLA = TCB_CLKSEL_CLKDIV1_gc; // No prescaling
    }
    // Set TCB0 to periodic interrupt mode (the mode where the counter resets on compare match)
    TCB0.CTRLB = TCB_CNTMODE_INT_gc;
    TCB0.CCMP = compare_val;           // Set the compare value
    TCB0.INTCTRL = TCB_CAPTEI_bm;        // Enable the compare match interrupt flag
